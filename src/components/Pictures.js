...
function Pictures(props) {
  const baseURL =
    "https://ap-south-1.aws.data.mongodb-api.com/app/application-0-wzbdi/endpoint/pics";
        ...
        data
          ...
          .map((info) => (
            <div className="flex flex-col m-5" key={info.id}>
              {info.year ? (
                <div
                  className="bg-secondary text-highlight text-center -mb-6 text-[3.5vh] border-t-8 border-highlight flex justify-center z-10 rounded-xl font-headingFont"
                  id="groupImg "
                >
                  {info.year}
                </div>
              ...
              <img
                src={`https://drive.google.com/uc?export=view&id=` + info.pic}
                alt={info.id}
                key={info.id}
                id="groupImg"
                className="w-auto rounded-xl md:h-[50vh] h-[25vh] m-2 border-8 border-secondary mb-4"
              />
              {info.contact ? (
                <div
                  className="bg-secondary text-highlight text-center -mt-6 text-[3.5vh] border-b-8 border-highlight flex justify-center rounded-xl"
                  id="groupImg"
                >
                  <a
                  ...
                  >
                    <svg
                      className="m-2 shadow-md shadow-background rounded-full cursor-pointer hover:scale-125 duration-200"
                      width="50px"
                      height="50px"
                      viewBox="-7.5 -7.5 40.00 40.00"
                      fill="none"
