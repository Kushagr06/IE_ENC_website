...
<div className='container h-[25rem] lg:w-1/5 md:w-1/3 w-3/4 bg-secondary rounded-2xl border-4 border-primary sm:scale-100 scale-[0.85]  sm:m-10 p-5 flex flex-col justify-center' key={data.id}> 
    {/* image */}
    <img src={`https://drive.google.com/uc?export=view&id=`+data.image} alt="Pic" className='w-full  bg-cover  max-w-full rounded-b-[6rem] relative -top-6 rounded-t-2xl shadow-md shadow-background'/>
        {/* Join Link */}
        <a href={data.formLink} target='__blank'>
        <button className=' h-16 w-16 rounded-full 
           bg-highlight duration-200 hover:scale-110 hover:my-1 font-semibold mt-2 z-10 relative bottom-28 text-2xl shadow-md shadow-background content-center flex justify-center items-center'> 
        ...
        {/* Event Name   */}
          ...
          <a href={data.instaLink} target='__blank' className='flex justify-center'>
            <span className=' inline-block'>
